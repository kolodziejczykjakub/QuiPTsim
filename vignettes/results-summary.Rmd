...
```
```{r}
tmp <- parse_results(paste0(data_path, "result__", "FCBF_nonranking", "_", num_reps, ".Rds"))
```



```{r}
paste0("./data/experiment-results/exp01-seqLen10-nSeq300-amylogram-encoding/", "result__", "FCBF_nonranking", "_", num_reps, ".Rds")
```
```{r}
...
Model = "16-NN"
metrics = "accuracy"
tmp <- lapply(nonranking_results, function(x){
  data.frame(x[x$Model == Model, c(paste0(metrics, "_mean"), paste0(metrics, "_std"))])
})
do.call(rbind, tmp)
```
...
total_results %>%
  ggplot(aes(x=n_kmers, y=auc_mean, group=Filter, color=Filter)) +

...
plan <- drake_plan(
  ...
  df = read.csv("~/projects/QuiPTsim-data/exp3_reduced_alph_enc_15motifs_amylogram_encoding/amylogram_encoding.csv"),
  ...
  ###### selected paths
  paths = paste0("~/projects/QuiPTsim-data/exp3_reduced_alph_enc_15motifs_amylogram_encoding/",
                 sapply(strsplit(x = df[df$l_seq==10 & df$n_motifs==2,"path"], split = "/"), function(x) x[[3]])),
  
  output_prefix = "~/experiment-results/exp3_reduced_alph_enc_15motifs_amylogram_encoding/exp03-nSeq300-nMotifs2-frac025/result_",
...
cache <- new_cache("exp03_15motifs_nSeq300_nMotifs2_frac025")
make(
  plan,
  parallelism = "future",
  jobs = 8,
  log_make = "exp03_15motifs_nSeq300_nMotifs2_frac025.log",
  cache = cache,
  seed = 42
)

...
plan <- drake_plan(
  ...
  paths = sample(paste0("~/projects/QuiPTsim-data/reduced_alph_enc_amylogram_encoding_unigram/",
                 ...
                 size = size),
  output_prefix = "~/experiment-results/exp01-seqLen10-nSeq600-frac025-amylogram-encoding/result_",
...
cache <- new_cache("exp01_seqLen10_nSeq600_frac025_amylogram_encoding")
make(
  plan,
  parallelism = "future",
  jobs = 8,
  log_make = "exp01_seqLen10_nSeq600_frac025_amylogram_encoding.log",
  cache = cache,
  seed = 42
)
